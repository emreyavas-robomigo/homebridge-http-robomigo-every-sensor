# homebridge-http-robomigo-every-sensor

This [Homebridge](https://github.com/nfarina/homebridge) plugin can be used integrate your every sensor which has a 
...
instructions.  
Then run the following command to install `homebridge-http-robomigo-every-sensor`

```
sudo npm install -g emreyavas-robomigo/homebridge-http-robomigo-every-sensor
```

## Updating the Accessory Level in HomeKit

The _'CarbonDioxideLevel'_ characteristic has the permission to `notify` the HomeKit controller of state 
changes. `homebridge-http-robomigo-every-sensor` supports two concepts to send every Level changes to HomeKit.

### The 'pull' way:

The 'pull' way is probably the easiest to set up and supported in every scenario. `homebridge-http-robomigo-every-sensor` 
requests the every Level of the sensor in an specified interval (pulling) and sends the value to HomeKit.  
Look for `pullInterval` in the list of configuration options if you want to configure it.

### The 'push' way:

When using the 'push' concept, the http device itself sends the updated value to `homebridge-http-robomigo-every-sensor` 
whenever values change. This is more efficient as the new value is updated instantly and 
`homebridge-http-robomigo-every-sensor` does not need to make needless requests when the value didn't actually change.  
However because the http device needs to actively notify the `homebridge-http-robomigo-every-sensor` there is more 
...
* `accessory` \<string\> **required**: Defines the plugin used and must be set to **"HTTP-EVERY"** for this plugin.
* `name` \<string\> **required**: Defines the name which is later displayed in HomeKit
* `getUrl` \<string |  [urlObject](#urlobject)\> **required**: Defines the url (and other properties when using 
    and urlObject) to query the current every Level from the sensor. 

##### Advanced configuration options:

* `statusPattern` \<string\> **optional** \(Default: **"/([0-9]{1,3})/"**): Defines a regex pattern with which the 
    every Level is extracted from the body of the http response from the `getUrl`. The group which should
...
* `patternGroupToExtract` <\number\> **optional** \(Default: **1**\): Defines the regex group of which the every Level 
    is extracted.
...
```json
{
    "accessories": [
        {
          "accessory": "HTTP-EVERY",
          "name": "every Sensor",
          
          "getUrl": "http://localhost/getevery"
        }   
    ]
}
```
...
Below is an example of an mqttObject containing the basic properties for a every Level service:
```json
{
  "host": "127.0.0.1",
  "port": "1883",
...
`homebridge-http-robomigo-every-sensor` can be used together with 
...
Down here is an example on how to configure `homebridge-http-robomigo-every-sensor` to work with your implementation of the 
    ...
    "accessories": [
        {
          ...
          "notificationID": "my-every-sensor",
          "notificationPassword": "superSecretPassword",
          
          "getUrl": "http://localhost/api/getevery"
        }   
    ]
}
```
...
Any value that is sent will be accepted. Like CarbonDioxideLevel CarbonMonoxideLevel AirQuality CurrentTemperature ContactSensorState
